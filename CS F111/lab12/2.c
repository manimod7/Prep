...
void addDays(struct Date dt, int x)
{
  ...
  printf("The final date: %d/%d/%d", ans.d, ans.m, ans.y);
}
int main()
{
  struct Date dt;
  ...
  dt.y = t->tm_year + 1900;
  int x;
  printf("The current date: %d/%d/%d", dt.d, dt.m, dt.y);
  printf("\nPlease enter the number of days(>0) to add: ");
  scanf("%d", &x);
  addDays(dt, x);
  return 0;
}